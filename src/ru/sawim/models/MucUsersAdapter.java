...
public class MucUsersAdapter extends BaseAdapter {
    ...
    void populateFrom(final RosterItemView rosterItemView, Xmpp protocol, XmppContact.SubContact c) {
        ...
        if (ic != null && !SawimApplication.hideIconsClient) {
            rosterItemView.itemFifthImage = ((BitmapDrawable)ic.getImage()).getBitmap();
        }
        rosterItemView.itemFourthImage = ((BitmapDrawable)SawimResources.affiliationIcons.iconAt(XmppServiceContact.getStatusName(c.priorityA)).getImage()).getBitmap();
    }
}
