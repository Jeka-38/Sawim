...
public class RosterAdapter extends BaseAdapter implements View.OnClickListener{
    ...
    void populateFromProtocol(RosterItemView rosterItemView, ProtocolBranch o) {
        ...
        rosterItemView.itemSecondImage = ((BitmapDrawable)new Icon(o.isExpanded() ?
        ...
        if (icGroup != null)
            rosterItemView.itemThirdImage = ((BitmapDrawable)icGroup.getImage()).getBitmap();
        ...
        if (profile != null) {
            if (profile.xstatusIndex != XStatusInfo.XSTATUS_NONE) {
                ...
                if (xStatusInfo != null) {
                    ...
                    if (xStatusIcon != null)
                        rosterItemView.itemFourthImage = ((BitmapDrawable)xStatusIcon.getImage()).getBitmap();
                }
            }
        }
    ...
    void populateFromGroup(RosterItemView rosterItemView, Group g) {
        ...
        rosterItemView.itemFirstImage = ((BitmapDrawable)new Icon(g.isExpanded() ?
                SawimResources.groupDownIcon : SawimResources.groupRightIcons).getImage()).getBitmap();
    ...
    void populateFromContact(final RosterItemView rosterItemView, RosterHelper roster, Protocol p, Contact item) {
       ...
       // rosterItemView.itemSixthImage = ((BitmapDrawable)SawimResources.affiliationIcons.iconAt(XmppServiceContact.getStatusName(item.getStatusIndex())).getImage()).getBitmap();
        ...
        if (icClient != null && !SawimApplication.hideIconsClient)
            rosterItemView.itemFifthImage = ((BitmapDrawable)icClient.getImage()).getBitmap();
        if (item.isTyping()) {
        ...
        if (item.getXStatusIndex() != XStatusInfo.XSTATUS_NONE) {
            ...
            if (xStatusInfo != null)
                rosterItemView.itemThirdImage = ((BitmapDrawable)xStatusInfo.getIcon(item.getXStatusIndex()).getImage()).getBitmap();
        }
        ...
        if (!item.isTemp()) {
            if (item.isAuth()) {
                ...
                if (privacyList != -1)
                    rosterItemView.itemThirdImage = ((BitmapDrawable)Contact.serverListsIcons.iconAt(privacyList).getImage()).getBitmap();
            } else {
    ...
    public BitmapDrawable getImageChat(Chat chat, boolean showMess) {
        if (chat.getContact().isTyping()) {
            ...
            return icMess == null || !showMess ? (BitmapDrawable)(icStatus.getImage()) : icMess;
        }
    }
